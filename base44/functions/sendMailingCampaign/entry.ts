...
function buildEmailHtml({ subject, headline, body, cta, image_url }) {
  const imgBlock = image_url
    ? `<img src="${image_url}" alt="" style="width:100%;max-height:280px;object-fit:cover;display:block;"/>`
    : '';
  const ctaBlock = cta?.label
    ? `<table cellpadding="0" cellspacing="0" border="0" style="margin:32px auto 0;"><tr><td style="border-radius:6px;background:${PRIMARY};"><a href="${cta.url || SITE_URL}" style="display:inline-block;padding:14px 32px;font-family:'Helvetica Neue',Arial,sans-serif;font-size:15px;font-weight:700;color:#ffffff;text-decoration:none;">${cta.label}</a></td></tr></table>`
    : '';
...
Deno.serve(async (req) => {
  try {
    ...
    if (audience === 'custom') {
      // Emails saisis manuellement (virgule ou saut de ligne)
    ...
    const htmlBody = buildEmailHtml({ subject, headline, body, cta, image_url });
